...
def make_dataset( annotation_path, video_path , whole_path,sample_duration, n_samples_for_each_video, stride_len):
    whole_path = whole_path.replace(".avi", "")
    data = load_annotation_data(annotation_path)
    # print(data, whole_path)
    whole_video_path = os.path.join(video_path, whole_path)
    # print("whole_video_path", whole_video_path)
    ...
    import glob
    whole_video_path = whole_video_path.replace("videos", "images")
    ...
    for k in range(index+1, n_frames+1):
        label_list.append(84)
